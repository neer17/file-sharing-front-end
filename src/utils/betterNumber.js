...
export const betterNumber = (fileInBytes, inFormat = false) => {
  ...
  if (fileInBytes === 0) return 0

  if (fileInBytes < kb) {
    if (inFormat)
      return (fileFormat = String(Math.round(fileInBytes / kb)).concat("KB"))
    else return (betterFormat = Math.round(fileInBytes / kb))
  }
  if (fileInBytes > kb) {
    if (inFormat)
      return (fileFormat = String(Math.round(fileInBytes / kb)).concat("KB"))
  ...
  if (fileInBytes > mb) {
    ...
    else return (betterFormat = Math.round(fileInBytes / mb))
  }
  if (fileInBytes > gb) {
    if (inFormat)
      return (fileFormat = String(Math.round(fileInBytes / gb)).concat("GB"))
    else return (betterFormat = Math.round(fileInBytes / gb))
  }
}
